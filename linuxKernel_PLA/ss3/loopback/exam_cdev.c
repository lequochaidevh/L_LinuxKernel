...
#define DEVICE		5
#define AUTHOR		"EMBEDDED LINUX PLA"
...
{
	printk(KERN_INFO "LinuxInfo %s, %d\n", __func__, __LINE__);
	first_oper = true;
...
static long my_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	case SEND_DATA_CMD:
		...
		printk(KERN_INFO "LinuxInfo %s, %d, ioctl message = %s\n", __func__, __LINE__, config_data);
		break;
...
static int __init func_init(void)
{
	...
	pr_info("register succesfully device with minor %s\n", DEVICE_NAME);
	return 0;
}
