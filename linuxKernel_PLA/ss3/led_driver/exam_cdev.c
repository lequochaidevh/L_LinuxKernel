...
{
	printk(KERN_INFO "LED_DRIVER %s, %d\n", __func__, __LINE__);
...
static ssize_t my_write(struct file *flip, const char __user *user_buf, size_t len, loff_t *offs)
{
	...
	switch (data) {
		...
		case '0':
			...
			break;
		default:
			printk(KERN_INFO "LED_DRIVER un-support operation");
	};
...
static long my_ioctl(struct file *filep, unsigned int cmd, unsigned long arg)
{
	...
	case SEND_DATA_CMD:
		...
		printk(KERN_INFO "LED_DRIVER %s, %d, ioctl message = %s\n", __func__, __LINE__, config_data);
		break;
